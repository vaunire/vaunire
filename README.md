...
2. **Настройте окружение:**
   - Убедитесь, что PostgreSQL запущен и создана база данных, указанная в `.env`.
   - **Redis:** Если вы запускаете проект без Docker и у вас нет локального Redis, установите `USE_REDIS=False` в файле `.env`. В противном случае убедитесь, что Redis сервер доступен.
